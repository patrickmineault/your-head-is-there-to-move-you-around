...
class V1Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.s1 = ShallowNet(
            nstartfeats=64, symmetric=True, dropout_rate=0, weight_norm=False
        )

        self.skip_conv = nn.Conv3d(64, 32, [1, 1, 1], [1, 1, 1])
        self.bn = nn.BatchNorm3d(32)
        self.relu = nn.LeakyReLU(negative_slope=0.01)

        self.res0 = ResBlock(
            64, 32, 3, 1, BottleneckTransform, 8, drop_connect_rate=0.2
        )

        self.res1 = ResBlock(
            32, 32, 3, 1, BottleneckTransform, 8, drop_connect_rate=0.2
        )
