...
def main():
    ...
    args = wrap({"data_root": "../data_derived", "subset": 0, "dataset": "pvc4"})
    traintune = models.get_dataset(args, "traintune")
    reportset = models.get_dataset(args, "report")
    ...
    args = wrap({"data_root": "../data_derived", "subset": 6, "dataset": "pvc4"})
    traintune = models.get_dataset(args, "traintune")
    reportset = models.get_dataset(args, "report")
    ...
    traintune_seq = {
        "../data_derived/crcns-pvc4/Nat/r0214A/test.review.mountlake.20_pix.50sec.imsm": np.zeros(
            3600, dtype=np.bool
        )
    }
    reportset_seq = {
    ...
    for s in traintune.sequence:
        if s["images_path"] in traintune_seq:
            traintune_seq[s["images_path"]][
                s["start_spktime"] : s["end_spktime"]
            ] = True

    for s in reportset.sequence:
        if s["images_path"] in reportset_seq:
            reportset_seq[s["images_path"]][
                s["start_spktime"] : s["end_spktime"]
            ] = True
    ...
    assert (
        np.sum(
            traintune_seq[
                "../data_derived/crcns-pvc4/Nat/r0214A/test.review.mountlake.20_pix.50sec.imsm"
            ]
            & reportset_seq[
                "../data_derived/crcns-pvc4/Nat/r0214A/test.review.mountlake.20_pix.50sec.imsm"
            ]
